...
{
    public class ClientNotDispReport
    {
        ...
        public ClientNotDispReport()
        {
            FrontsDT = new DataTable();
            DecorDT = new DataTable();
        }

        public void FillTables(DateTime date2, DateTime time2, ArrayList MClients, ArrayList MClientGroups)
        {
            ...
            string Filter = @$" AND Packages.ProductType = 0 AND (DispatchDateTime is null or DispatchDateTime >= '{date2:yyyy-MM-dd} {time2:HH:mm:ss}')";
            ...
            string SelectCommand = @$"SELECT infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber, Packages.PackingDateTime, dbo.PackageDetails.PackageID, dbo.PackageDetails.PackNumber, dbo.FrontsOrders.FrontsOrdersID,
                        dbo.PackageDetails.Count, FrontsOrders.Square*dbo.PackageDetails.Count/FrontsOrders.Count as Square, FrontsOrders.FactoryID,
                         ...
                         InsetTypes.TechStoreName AS Expr36, InsetColors.TechStoreName AS Expr37, 
TechnoInsetTypes.TechStoreName AS Expr38, TechnoInsetColors.TechStoreName AS Expr1,FrontsOrders.CreateDateTime, ZOVClientID
...
where FrontsOrders.CreateDateTime <= '{date2:yyyy-MM-dd} {time2:HH:mm:ss}'
ORDER BY infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber, dbo.PackageDetails.PackageID";
            ...
            SelectCommand = @$"SELECT infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber, null, null, null, dbo.FrontsOrders.FrontsOrdersID,
                         dbo.FrontsOrders.Count, FrontsOrders.Square, FrontsOrders.FactoryID,
                         infiniu2_catalog.dbo.FrontsConfig.AccountingName, infiniu2_catalog.dbo.FrontsConfig.InvNumber, infiniu2_catalog.dbo.Measures.Measure, Fronts.TechStoreName, Colors.TechStoreName AS Expr35,
                         InsetTypes.TechStoreName AS Expr36, InsetColors.TechStoreName AS Expr37, 
TechnoInsetTypes.TechStoreName AS Expr38, TechnoInsetColors.TechStoreName AS Expr1, FrontsOrders.CreateDateTime, ZOVClientID
FROM dbo.FrontsOrders LEFT OUTER JOIN
                         JoinMainOrders ON dbo.FrontsOrders.MainOrderID = JoinMainOrders.MarketMainOrderID LEFT OUTER JOIN
                         infiniu2_catalog.dbo.TechStore AS Fronts ON dbo.FrontsOrders.FrontID = Fronts.TechStoreID LEFT OUTER JOIN
                         infiniu2_catalog.dbo.TechStore AS Colors ON dbo.FrontsOrders.ColorID = Colors.TechStoreID LEFT OUTER JOIN
                         infiniu2_catalog.dbo.TechStore AS InsetTypes ON dbo.FrontsOrders.InsetTypeID = InsetTypes.TechStoreID LEFT OUTER JOIN
                         ...
                         infiniu2_catalog.dbo.FrontsConfig ON dbo.FrontsOrders.FrontConfigID = infiniu2_catalog.dbo.FrontsConfig.FrontConfigID INNER JOIN
                         infiniu2_catalog.dbo.Measures ON infiniu2_catalog.dbo.FrontsConfig.MeasureID = infiniu2_catalog.dbo.Measures.MeasureID INNER JOIN
                         dbo.MainOrders ON dbo.FrontsOrders.MainOrderID = dbo.MainOrders.MainOrderID 
...
ORDER BY infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber";
            ...
            {
                FrontsDT.ImportRow(dt.Rows[i]);
            }

            using (DataView DV = new DataView(FrontsDT.Copy(), string.Empty, "ClientName, OrderNumber", DataViewRowState.CurrentRows))
            {
                FrontsDT.Clear();
                FrontsDT = DV.ToTable();
            }
            ...
            Filter = @$" AND Packages.ProductType = 1 AND (DispatchDateTime is null or DispatchDateTime >= '{date2:yyyy-MM-dd} {time2:HH:mm:ss}')";
            ...
            SelectCommand = @$"SELECT infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber, Packages.PackingDateTime, dbo.PackageDetails.PackageID, dbo.PackageDetails.PackNumber, dbo.DecorOrders.DecorOrderID,
                         DecorOrders.FactoryID, infiniu2_catalog.dbo.DecorConfig.AccountingName, infiniu2_catalog.dbo.DecorConfig.InvNumber,
infiniu2_catalog.dbo.Measures.Measure, Fronts.TechStoreName, Colors.TechStoreName AS Expr35, 
dbo.DecorOrders.Length, dbo.DecorOrders.Height, dbo.DecorOrders.Width, dbo.PackageDetails.Count, DecorOrders.CreateDateTime, ZOVClientID
FROM dbo.PackageDetails INNER JOIN
                        ...
                        infiniu2_catalog.dbo.Measures ON infiniu2_catalog.dbo.DecorConfig.MeasureID = infiniu2_catalog.dbo.Measures.MeasureID INNER JOIN
                         ...
                         dbo.MegaOrders ON dbo.MainOrders.MegaOrderID = dbo.MegaOrders.MegaOrderID {MClientFilter} INNER JOIN
                         infiniu2_marketingreference.dbo.Clients ON dbo.MegaOrders.ClientID = infiniu2_marketingreference.dbo.Clients.ClientID
where decorOrders.CreateDateTime <= '{date2:yyyy-MM-dd} {time2:HH:mm:ss}'
ORDER BY infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber, dbo.PackageDetails.PackageID";
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                DA.Fill(DecorDT);
            ...
            SelectCommand = @$"SELECT infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber,  null, null, null, dbo.DecorOrders.DecorOrderID,
                         DecorOrders.FactoryID, infiniu2_catalog.dbo.DecorConfig.AccountingName, infiniu2_catalog.dbo.DecorConfig.InvNumber, infiniu2_catalog.dbo.Measures.Measure, 
Fronts.TechStoreName, Colors.TechStoreName AS Expr35, dbo.DecorOrders.Length, dbo.DecorOrders.Height, dbo.DecorOrders.Width, dbo.DecorOrders.Count, DecorOrders.CreateDateTime, ZOVClientID
FROM dbo.DecorOrders LEFT OUTER JOIN
                         JoinMainOrders ON dbo.DecorOrders.MainOrderID = JoinMainOrders.MarketMainOrderID LEFT OUTER JOIN
                         infiniu2_catalog.dbo.TechStore AS Fronts ON dbo.DecorOrders.DecorID = Fronts.TechStoreID LEFT OUTER JOIN
                         infiniu2_catalog.dbo.TechStore AS Colors ON dbo.DecorOrders.ColorID = Colors.TechStoreID INNER JOIN
                         infiniu2_catalog.dbo.DecorConfig ON dbo.DecorOrders.DecorConfigID = infiniu2_catalog.dbo.DecorConfig.DecorConfigID INNER JOIN
                        infiniu2_catalog.dbo.Measures ON infiniu2_catalog.dbo.DecorConfig.MeasureID = infiniu2_catalog.dbo.Measures.MeasureID INNER JOIN
                         dbo.MainOrders ON dbo.DecorOrders.MainOrderID = dbo.MainOrders.MainOrderID 
and not (ProfilProductionStatusID = 1 AND ProfilStorageStatusID = 1 AND ProfilExpeditionStatusID = 1 AND ProfilDispatchStatusID = 2)
and MainOrders.mainorderid not in (select packages.mainorderid from packages)
INNER JOIN
                         dbo.MegaOrders ON dbo.MainOrders.MegaOrderID = dbo.MegaOrders.MegaOrderID {MClientFilter} INNER JOIN
...
ORDER BY infiniu2_marketingreference.dbo.Clients.ClientName, dbo.MegaOrders.OrderNumber";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                DecorDT.ImportRow(dt.Rows[i]);
            }
            using (DataView DV = new DataView(DecorDT.Copy(), string.Empty, "ClientName, OrderNumber", DataViewRowState.CurrentRows))
            {
                DecorDT.Clear();
                DecorDT = DV.ToTable();
            }
        ...
        public void Report(string FileName)
        {
            ...
            if (FrontsProfilDT.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < FrontsProfilDT.Rows.Count; i++)
                {
                    ...
                    Cell1.CellStyle = SimpleCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(ColIndex++);
                    if (FrontsProfilDT.Rows[i]["PackageID"] == DBNull.Value)
                        Cell1.SetCellValue("");
                    else
                    ...
                    if (FrontsProfilDT.Rows[i]["PackNumber"] == DBNull.Value)
                        Cell1.SetCellValue("");
                    else
                        Cell1.SetCellValue(Convert.ToInt32(FrontsProfilDT.Rows[i]["PackNumber"]));
                    Cell1.CellStyle = CountCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(ColIndex++);
                    Cell1.SetCellValue(FrontsProfilDT.Rows[i]["CreateDateTime"].ToString());
                    Cell1.CellStyle = SimpleCS;
            ...
            if (DecorProfilDT.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < DecorProfilDT.Rows.Count; i++)
                {
                    ...
                    Cell1.CellStyle = SimpleCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(ColIndex++);
                    if (DecorProfilDT.Rows[i]["packageid"] == DBNull.Value)
                        Cell1.SetCellValue("");
                    else
                        Cell1.SetCellValue(Convert.ToInt32(DecorProfilDT.Rows[i]["packageid"]));
                    Cell1.CellStyle = CountCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(ColIndex++);
                    if (DecorProfilDT.Rows[i]["PackNumber"] == DBNull.Value)
                        Cell1.SetCellValue("");
                    else
                    ...
                    Cell1 = sheet1.CreateRow(pos).CreateCell(ColIndex++);
                    Cell1.SetCellValue(DecorProfilDT.Rows[i]["CreateDateTime"].ToString());
                    Cell1.CellStyle = SimpleCS;
                    pos++;
                }
            }
