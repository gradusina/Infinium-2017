    ...
    public class FrontsReport
    {
        ...
        private void GetCurvedFronts(DataTable OrdersDataTable, DataTable ReportDataTable)
        {
            ...
            for (int i = 0; i < Fronts.Rows.Count; i++)
            {
                ...
                Cost = Math.Ceiling(Solid713Count * Solid713Price / 0.01m) * 0.01m;
        ...
        public void Report(int[] DispatchID, int CurrencyTypeID, bool ProfilVerify, bool TPSVerify, int ClientID, int DiscountPaymentConditionID)
        {
            ...
            SelectCommand = @"SELECT DISTINCT PackageDetailID, PackageDetails.Count AS Count, 
                (FrontsOrders.Square * PackageDetails.Count / FrontsOrders.Count) AS Square, 
                (FrontsOrders.Cost * PackageDetails.Count / FrontsOrders.Count) AS Cost, 
                (FrontsOrders.CostWithTransport * PackageDetails.Count / FrontsOrders.Count) AS CostWithTransport, 
                FrontsOrders.*, infiniu2_catalog.dbo.FrontsConfig.measureId, infiniu2_catalog.dbo.FrontsConfig.AccountingName, 
                infiniu2_catalog.dbo.FrontsConfig.InvNumber, MegaOrders.Rate, MegaOrders.PaymentRate FROM PackageDetails
