...
namespace Infinium.Modules.ExpeditionMarketing.ColorDispatchReportToDbf
{
    public class ColorInvoiceReportToDbf
    {
        ...
        public ColorDispatchDecorReportToDbf DispatchDecorReport;
        public ColorDispatchFrontsReportToDbf DispatchFrontsReport;
        ...
        public ColorInvoiceReportToDbf(FrontsCatalogOrder FrontsCatalogOrder, DecorCatalogOrder DecorCatalogOrder)
        {
            DispatchFrontsReport = new ColorDispatchFrontsReportToDbf(FrontsCatalogOrder, DecorCatalogOrder);
            DispatchDecorReport = new ColorDispatchDecorReportToDbf(DecorCatalogOrder);
        ...
        public void Collect(DataTable table1, DataTable table2, bool IsNonStandard)
        {
            ...
            for (int i = 0; i < DistinctInvNumbersDT.Rows.Count; i++)
            {
                ...
                DataRow[] InvRows = table1.Select("Cvet = '" + Cvet + "' AND Patina = '" + Patina +
                                                  "' AND InvNumber = '" + InvNumber + "' AND PaymentRate='" + PaymentRate + "' AND AccountingName='" + AccountingName + "'" + IsNonStandardFilter);
                if (InvRows.Count() > 0)
                {
...
PackageDetails.PackageID, infiniu2_catalog.dbo.FrontsConfig.InvNumber, infiniu2_catalog.dbo.FrontsConfig.FactoryID, Packages.DispatchID
                ...
                GROUP BY PackageDetails.PackageID, infiniu2_catalog.dbo.FrontsConfig.InvNumber, Cvet, Patina, infiniu2_catalog.dbo.FrontsConfig.FactoryID, Packages.DispatchID",
...
PackageDetails.PackageID, infiniu2_catalog.dbo.DecorConfig.InvNumber, infiniu2_catalog.dbo.DecorConfig.FactoryID, Packages.DispatchID
                ...
                GROUP BY PackageDetails.PackageID, infiniu2_catalog.dbo.DecorConfig.InvNumber, Cvet, Patina, infiniu2_catalog.dbo.DecorConfig.FactoryID, Packages.DispatchID",
                 ConnectionStrings.MarketingOrdersConnectionString))
            {
                TempDT.Clear();
                DA.Fill(TempDT);
            ...
            for (int z = 0; z < DispatchID.Count(); z++)
            {
                ...
                {
                    dtMainOrders.Clear();
                    dtMainOrders = DV.ToTable(true, new string[] { "PackageID", "InvNumber", "Cvet", "Patina" });
                }
                ...
                for (int i = 0; i < DistPackagesDT.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < rows1.Count(); j++)
                    {
                        ...
                        DataRow[] rows2 = ProfilReportTable.Select("InvNumber='" + InvNumber + "' AND Cvet='" + Cvet + "' AND Patina='" + Patina + "'");
                        if (rows2.Count() > 0)
                        {
                            if (j == rows1.Count() - 1)
                            {
                ...
                for (int i = 0; i < DistPackagesDT.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < rows1.Count(); j++)
                    {
                        ...
                        string Cvet = rows1[j]["Cvet"].ToString();
                        string Patina = rows1[j]["Patina"].ToString();
                        DataRow[] rows2 = TPSReportTable.Select("InvNumber='" + InvNumber + "' AND Cvet='" + Cvet + "' AND Patina='" + Patina + "'");
                        if (rows2.Count() > 0)
                        {
                            if (j == rows1.Count() - 1)
                            {
            ...
            if (ProfilReportTable.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ProfilReportTable.Rows.Count; i++)
                {
                    ...
                    Cell1.CellStyle = SimpleCS;
                    //Cell1 = sheet1.CreateRow(pos).CreateCell(displayIndex++);
                    //Cell1.SetCellValue(DispatchFrontsReport.GetColorNameByCode(ProfilReportTable.Rows[i]["Cvet"].ToString()));
                    //Cell1.CellStyle = SimpleCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(displayIndex++);
                    Cell1.SetCellValue(DispatchFrontsReport.GetPatinaNameByCode(ProfilReportTable.Rows[i]["Patina"].ToString()));
                    Cell1.CellStyle = SimpleCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(displayIndex++);
            ...
            if (TPSReportTable.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < TPSReportTable.Rows.Count; i++)
                {
                    ...
                    Cell1.CellStyle = SimpleCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(displayIndex++);
                    Cell1.SetCellValue(DispatchFrontsReport.GetColorNameByCode(TPSReportTable.Rows[i]["Cvet"].ToString()));
                    Cell1.CellStyle = SimpleCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(displayIndex++);
                    Cell1.SetCellValue(DispatchFrontsReport.GetPatinaNameByCode(TPSReportTable.Rows[i]["Patina"].ToString()));
                    Cell1.CellStyle = SimpleCS;
                    Cell1 = sheet1.CreateRow(pos).CreateCell(displayIndex++);
            ...
            for (int z = 0; z < DispatchID.Count(); z++)
            {
                ...
                for (int i = 0; i < DistPackagesDT.Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < rows1.Count(); j++)
                    {
                        ...
                        {
                            Console.WriteLine($"Найдено 0 упаковок. InvNumber={InvNumber} PackageID={PackageID}");
                        }
                    }
                }
