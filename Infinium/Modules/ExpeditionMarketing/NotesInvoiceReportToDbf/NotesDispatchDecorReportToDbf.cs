...
{
    public class NotesDispatchDecorReportToDbf
    {
        ...
        public NotesDispatchDecorReportToDbf(DecorCatalogOrder tDecorCatalogOrder)
        {
...
PackageDetailID, PackageDetails.Count AS Count, (DecorOrders.Cost * PackageDetails.Count / DecorOrders.Count) AS Cost, (DecorOrders.CostWithTransport * PackageDetails.Count / DecorOrders.Count) AS CostWithTransport, DecorOrders.*, infiniu2_catalog.dbo.DecorConfig.AccountingName, infiniu2_catalog.dbo.DecorConfig.InvNumber, MegaOrders.Rate, MegaOrders.PaymentRate FROM PackageDetails
        ...
        public int GetCubfurPackage(string invNumber)
        {
            DataRow[] row = decorCatalogOrder.DecorConfigDataTable.Select($"InvNumber = '{invNumber}'");
            if (!row.Any())
                return -1;

            int productId = Convert.ToInt32(row[0]["ProductId"]);
            int packageCount;
            if (Security.CabFurIds.Contains(productId))
                packageCount = Convert.ToInt32(row[0]["PackageCount"]);
            else
                return -1;
        ...
        private int GetReportMeasureTypeID(int DecorConfigID)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show(@$"NotesDispatchDecorReportToDbf Ошибка конфигурации: не найдена DecorConfigID={DecorConfigID}. 
