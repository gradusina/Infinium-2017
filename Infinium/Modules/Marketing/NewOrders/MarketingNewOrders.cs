...
namespace Infinium.Modules.Marketing.NewOrders
{
    ...
    public class FrontsOrders
    {
        ...
        private void Fill()
        {
            ...
            //                    WHERE TechStoreID IN (SELECT FrontID FROM FrontsConfig WHERE FrontConfigID IN 
            //                    (SELECT ConfigID FROM infiniu2_marketingreference.dbo.ExcluziveCatalog WHERE Enable=1 AND ProductType=0 AND ClientID=" + ClientID + @"))
            //                    ORDER BY TechStoreName";
            //            }
        ...
        private void CreateColumns()
        {
            ...
            //FrontsOrdersDataGrid.Columns["CupboardString"].AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
            //FrontsOrdersDataGrid.Columns["CupboardString"].Width = 85;
        ...
        public void FrontsOrdersSet(int MainOrderID, int CurrentMainOrderID)
        {
            ...
            DataRow[] FRows = FrontsOrdersDataTable.Select("MainOrderID=" + MainOrderID);
            foreach (DataRow FRow in FRows)
            {
                DataRow Row = FrontsOrdersDataTable.NewRow();

                Row.ItemArray = FRow.ItemArray;
            ...
            int InsetColorID, int TechnoProfileID, int TechnoColorID, int TechnoInsetTypeID, int TechnoInsetColorID, int Height, int Width, int Count, string Notes, int ImpostMargin = 0)
        {
            ...
            //{
            //    decimal WidthMin = TablesManager.GetWidthMin(InsetTypeID);
        ...
        public bool CanSaveFrontsOrder(ref int FactoryID)
        {
        ...
        public void SaveFrontsOrder(int MainOrderID)
        {
            for (int i = 0; i < FrontsOrdersDataTable.Rows.Count; i++)
            {
                if (FrontsOrdersDataTable.Rows[i].RowState == DataRowState.Deleted)
        ...
        public void ImportFromSizeTable(ref ComponentFactory.Krypton.Toolkit.KryptonDataGridView DataGrid)
        {
            DataGrid.AllowUserToAddRows = false;
    ...
    public class DecorOrders
    {
        ...
        private void GridSettings()
        {
            ...
            for (int i = 0; i < DecorCatalogOrder.DecorProductsCount; i++)
            {
                ...
                DecorItemOrdersDataGrids[i].CellFormatting += DecorOrders_CellFormatting;
            }
        ...
        public bool CanSaveDecorOrder(ref int FactoryID)
        {
            for (int i = 0; i < DecorCatalogOrder.DecorProductsCount; i++)
            {
        ...
        public void SaveDecorOrder(int MainOrderID)
        {
            DecorOrdersDataTable.Clear();
            DecorOrdersDataTable.AcceptChanges();
        ...
        public bool CanSaveDecorOrderSet(ref int FactoryID)
        {
        ...
        public void SaveDecorOrderSet(int MainOrderID)
        {
            for (int i = 0; i < DecorOrdersDataTable.Rows.Count; i++)
            {
                if (DecorOrdersDataTable.Rows[i].RowState == DataRowState.Deleted)
    ...
    public class MainOrdersFrontsOrders
    {
        ...
        private void CreateColumns(bool ShowPrice)
        {
            ...
            FrontsOrdersDataGrid.Columns["Count"].DisplayIndex = DisplayIndex++;
            FrontsOrdersDataGrid.Columns["OnStorage"].DisplayIndex = DisplayIndex++;
            FrontsOrdersDataGrid.Columns["OriginalPrice"].DisplayIndex = DisplayIndex++;
            FrontsOrdersDataGrid.Columns["ClientOriginalPrice"].DisplayIndex = DisplayIndex++;
            FrontsOrdersDataGrid.Columns["OriginalCost"].DisplayIndex = DisplayIndex++;
            FrontsOrdersDataGrid.Columns["CurrencyCost"].DisplayIndex = DisplayIndex++;
            ...
            FrontsOrdersDataGrid.Columns["IsSample"].HeaderText = "Образцы";
            FrontsOrdersDataGrid.Columns["OnStorage"].HeaderText = "Склад";
            FrontsOrdersDataGrid.Columns["ClientOriginalPrice"].HeaderText = "Цена в\r\nАгенте";
            ...
            FrontsOrdersDataGrid.Columns["ClientOriginalPrice"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
    ...
    public class MainOrdersDecorOrders
    {
        ...
        private void GridSettings(bool ShowPrice)
        {
            ...
            for (int i = 0; i < DecorCatalogOrder.DecorProductsCount; i++)
            {
                ...
                DecorItemOrdersDataGrids[i].Columns["IsSample"].HeaderText = "Образцы";
                DecorItemOrdersDataGrids[i].Columns["OnStorage"].HeaderText = "Склад";
                DecorItemOrdersDataGrids[i].Columns["ClientOriginalPrice"].HeaderText = "Цена в\r\nАгенте";
                ...
                DecorItemOrdersDataGrids[i].Columns["ClientOriginalPrice"].DisplayIndex = DisplayIndex++;
                ...
                DecorItemOrdersDataGrids[i].Columns["CurrencyCost"].DisplayIndex = DisplayIndex++;
                DecorItemOrdersDataGrids[i].Columns["CostWithTransport"].DisplayIndex = DisplayIndex++;
                DecorItemOrdersDataGrids[i].ColumnHeadersVisible = true;
            }
        }
        ...
        private int CurrentDecorOrderID = 0;
        ...
        {
            TestTechCatalogManager.GetDecorOrders(CurrentDecorOrderID,
            ...
            TestTechCatalogManager.CreateFrontsExcel(CurrentDecorOrderID.ToString(),
                Convert.ToInt32(((ComponentFactory.Krypton.Toolkit.KryptonContextMenuItem)sender).Tag));
        }
        ...
        private void MainOrdersDecorOrders_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                setOnStorage.Click += (o, args) =>
                {
                    ...
                    bool onStorage = Convert.ToBoolean(grid.SelectedRows[0].Cells["onStorage"].Value);
                    grid.SelectedRows[0].Cells["onStorage"].Value = !onStorage;
                };
    ...
    public class OrdersManager
    {
        ...
        public int CurrentDiscountPaymentConditionID = 1;
        ...
        private void MegaGridSetting()
        {
            ...
            MegaOrdersDataGrid.Columns["PlanDispDate"].DisplayIndex = DisplayIndex++;
            ...
            MegaOrdersDataGrid.Columns["PlanDispDate"].ReadOnly = false;
        }
        ...
        {
            OrdersTabControl.SuspendLayout();
        ...
        public decimal DiscountOrderSum(decimal TotalSum)
        {
            string withDot = TotalSum.ToString(System.Globalization.CultureInfo.InvariantCulture);
            decimal Discount = 0;
            DataRow[] Rows = DiscountOrderSumTable.Select("MinSum < " + withDot + " AND MaxSum >=" + withDot);
            if (Rows.Count() > 0)
        ...
        public decimal TransportCost(decimal Weight)
        {
            string withDot = Weight.ToString(System.Globalization.CultureInfo.InvariantCulture);
            decimal Cost = 0;
            DataRow[] Rows = TransportGridTable.Select("MinWeight < " + withDot + " AND MaxWeight >=" + withDot);
            if (Rows.Count() > 0)
                Cost = Convert.ToDecimal(Rows[0]["Cost"]);
            return Cost;
        }

        public decimal GetSelectedMainOrdersSquare()
        {
            decimal Square = 0;
            int[] NewMainOrders = GetSelectedMainOrders();

            if (NewMainOrders.Count() < 1)
                return 0;
        ...
        public decimal GetSelectedMegaOrdersSquare(
            bool bsOnProduction,
            bool bsNotInProduction,
            bool bsInProduction,
            bool bsInStorage,
        ...
        {
            decimal Square = 0;
            int[] NewMegaOrders = GetSelectedMegaOrders();
            string MainProductionStatus = string.Empty;

            if (NewMegaOrders.Count() < 1)
                return 0;
        ...
        public void CreateNewMegaOrder(int ClientID)
        {
               ...
               " ORDER BY OrderNumber", ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    ...
                    NewRow["ClientID"] = ClientID;
                    NewRow["OrderNumber"] = GetNextMegaNumber(ClientID);
                    NewRow["DelayOfPayment"] = GetDelayOfPayment(ClientID);
                    NewRow["OrderDate"] = Security.GetCurrentDate();
            ...
            CurrentMegaOrderID = Convert.ToInt32(((DataRowView)MegaOrdersBindingSource.Current).Row["MegaOrderID"]);
            FixOrderEvent(CurrentMegaOrderID, "Новый заказ");
        }
                    ...
                    " FROM NewMainOrders WHERE MainOrderID = " + MainOrderID, ConnectionStrings.MarketingOrdersConnectionString))
                {
                    using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                    {
                        using (DataTable DT = new DataTable())
                        {
                            ...
                            if (!IsCurrentMeganOrderCreatedByClient)
        ...
        public void SaveOrder(string Notes, bool IsSample, int FactoryID, bool NeedSetStatus)
        {
            for (int i = 0; i < 3; i++)
            {
                using (SqlDataAdapter DA = new SqlDataAdapter("SELECT MainOrderID, MegaOrderID, IsSample, Notes, FactoryID," +
                    ...
                    " FROM NewMainOrders WHERE MainOrderID = " + CurrentMainOrderID, ConnectionStrings.MarketingOrdersConnectionString))
                {
                    using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                    {
                        using (DataTable DT = new DataTable())
                        {
                            ...
                            if (FactoryID == 1)
                            {
                                if (Convert.ToInt32(DT.Rows[0]["TPSProductionStatusID"]) != 0)
                                {
                                    DT.Rows[0]["TPSProductionStatusID"] = 0;
                                    DT.Rows[0]["TPSStorageStatusID"] = 0;
                                    DT.Rows[0]["TPSExpeditionStatusID"] = 0;
        ...
        public void SetNotConfirmOrder()
        {
            if (MegaOrdersBindingSource.Count == 0)
                return;
        ...
        public void SetAgreementStatus(int AgreementStatusID)
        {
            if (MegaOrdersBindingSource.Count == 0)
                return;
        ...
        public void AcceptOrder()
        {
            ...
            {
                MessageBox.Show(ex.Message + " \r\nAcceptOrder КАКАЯ-ТО НЕВЕДОМАЯ ЙОБАНАЯ ХУЙНЯ!");
            }
        }
        ...
        public bool IsCurrentMeganOrderCreatedByClient
        {
            get
            {
        ...
        public int[] GetMainOrders(int[] NewMegaOrders)
        {
                ...
                " WHERE MegaOrderID IN (" + string.Join(",", NewMegaOrders) + ")", ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (DataTable DT = new DataTable())
                {
                    ...
                    for (int i = 0; i < DT.Rows.Count; i++)
                        NewMainOrders.Add(Convert.ToInt32(DT.Rows[i]["MainOrderID"]));
                }
            }
        ...
        public bool CanAddMainOrderToMegaOrder()
        {
            //if (!UpdateSelectedMegaOrder())
            //{
            //    MessageBox.Show("Один или несколько заказов были удалены, повторите операцию", "Ошибка");
            ...
            int OrderNumber = Convert.ToInt32(((DataRowView)MegaOrdersBindingSource.Current).Row["OrderNumber"]);
            int ClientID = Convert.ToInt32(((DataRowView)MegaOrdersBindingSource.Current).Row["ClientID"]);

            DataRow[] Row = MegaOrdersDataTable.Select("ClientID = " + ClientID + "AND OrderNumber = " + OrderNumber);

            int OrderStatusID = Convert.ToInt32(Row[0]["OrderStatusID"]);
        ...
        {
            if (((DataRowView)MegaOrdersBindingSource.Current).Row["AgreementStatusID"].ToString() == "1")
                return true;
        ...
        public bool CanRemoveMegaOrder()
        {
            ...
            //for (int i = 0; i < MainOrdersDataTable.Rows.Count; i++)
            //{
            //    int ProfilProductionStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["ProfilProductionStatusID"]);
            //    int ProfilStorageStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["ProfilStorageStatusID"]);
            //    int ProfilExpeditionStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["ProfilExpeditionStatusID"]);
            //    int ProfilDispatchStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["ProfilDispatchStatusID"]);
            //    int TPSProductionStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["TPSProductionStatusID"]);
            //    int TPSStorageStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["TPSStorageStatusID"]);
            //    int TPSExpeditionStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["TPSExpeditionStatusID"]);
            //    int TPSDispatchStatusID = Convert.ToInt32(MainOrdersDataTable.Rows[i]["TPSDispatchStatusID"]);

            //    int PS = ProfilProductionStatusID + ProfilStorageStatusID + ProfilExpeditionStatusID + ProfilDispatchStatusID;
            //    int TS = TPSProductionStatusID + TPSStorageStatusID + TPSExpeditionStatusID + TPSDispatchStatusID;

            //    if ((PS == 4 || PS == 0) && (TS == 4 || TS == 0))
            //        return true;
            //}
        ...
        public bool CanRemoveMainOrder()
        {
            ...
            return true;
            //int ProfilProductionStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["ProfilProductionStatusID"]);
            //int ProfilStorageStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["ProfilStorageStatusID"]);
            //int ProfilExpeditionStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["ProfilExpeditionStatusID"]);
            //int ProfilDispatchStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["ProfilDispatchStatusID"]);
            //int TPSProductionStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["TPSProductionStatusID"]);
            //int TPSStorageStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["TPSStorageStatusID"]);
            //int TPSExpeditionStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["TPSExpeditionStatusID"]);
            //int TPSDispatchStatusID = Convert.ToInt32(((DataRowView)MainOrdersBindingSource.Current)["TPSDispatchStatusID"]);

            ////заказ можно удалить до того, как он будет согласован, только если подзаказ не был в производстве
        ...
        public bool EditAgreedMainOrder()
        {
            NeedSetStatus = false;
            GetCurrentMainOrder();
        ...
        public void RemoveCurrentAgreedMainOrder()
        {
            if (MainOrdersBindingSource.Count < 1)
                return;
        ...
        public void RemoveCurrentMegaOrder()
        {
            ...
            if (OrderStatusID != 0)
            {
                MessageBox.Show("Заказ был отдан в производство и не может быть удален", "Ошибка удаления заказа");
                return;
            }
            ...
            for (int i = 0; i < NewMainOrders.Count(); i++)
            {
                MainOrdersFrontsOrders.DeleteOrder(NewMainOrders[i]);

                MainOrdersDecorOrders.DeleteOrder(NewMainOrders[i]);
                MainOrdersDecorOrders.DeleteDecorAssignmentByMainOrder(NewMainOrders[i]);
                DataRow[] Row = MainOrdersDataTable.Select("MainOrderID = " + NewMainOrders[i]);
        ...
        public void RemoveCurrentAgreedMegaOrder()
        {
            int pos = MegaOrdersBindingSource.Position;
                ...
                " FROM NewMegaOrders WHERE MegaOrderID = " + MegaOrderID, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    using (DataTable DT = new DataTable())
                    {
                        ...
                        if (DT.Rows.Count > 0)
                        {
                            bool CreatedByClient = Convert.ToBoolean(DT.Rows[0]["CreatedByClient"]);
                            if (CreatedByClient)
        ...
        public decimal GetMainOrdersCost(int[] NewMainOrders)
        {
            ...
            //MessageBox.Show("Выполнена функция GetMainOrdersCost");
            return OrderCost;
        }

        public string GetDailyRates(int ClientID)
        {
            ...
            using (SqlDataAdapter DA = new SqlDataAdapter("SELECT MegaOrderID, LastCalcDate, LastCalcUserID FROM NewMegaOrders WHERE MegaOrderID = " + MegaOrderID, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    using (DataTable DT = new DataTable())
                    {
                        ...
                        if (DT.Rows.Count > 0)
                        {
                            DT.Rows[0]["LastCalcDate"] = Security.GetCurrentDate();
                            DT.Rows[0]["LastCalcUserID"] = Security.CurrentUserID;
                            DA.Update(DT);
                        }
                    }
                }
                            ...
                            ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    using (DataTable DT = new DataTable())
                    {
                        ...
                        {
                            if (DesireDate.Length == 0)
                                DT.Rows[0]["DesireDate"] = DBNull.Value;
                            else
                                DT.Rows[0]["DesireDate"] = DesireDate;
                ...
                ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    using (DataTable DT = new DataTable())
                    {
                        ...
                        if (DT.Rows.Count > 0)
                        {
                            DT.Rows[0]["FixedPaymentRate"] = IsFixedPaymentRate;
                            DA.Update(DT);
                        }
                    }
                }
        ...
        public Tuple<bool, decimal, decimal, decimal, decimal> GetDateRates(DateTime dateTime)
        {
            string selectCommandTExt = $@"SELECT * FROM DateRates WHERE CAST(Date AS Date) = 
                    '{dateTime:yyyy-MM-dd}'";
        ...
        public decimal GetDateEuroRate(DateTime DateTime)
        {
            decimal BYN = 1;
            ...
            using (DataTable DT = new DataTable())
            {
                using (SqlDataAdapter DA = new SqlDataAdapter($@"SELECT * FROM DateRates WHERE CAST(Date AS Date) = 
                    '{DateTime.ToString("yyyy-MM-dd")}'", ConnectionStrings.MarketingReferenceConnectionString))
                {
                    DA.Fill(DT);
                    if (DT.Rows.Count > 0)
                    {
        ...
        public void FixOrderEvent(int MegaOrderID, string Event)
        {
        ...
        public void MoveOrdersTo(int MegaOrderID, int AgreementStatusID)
        {
            DataTable TempDT = new DataTable();
            string SelectCommand = @"SELECT * FROM NewMegaOrders WHERE MegaOrderID = " + MegaOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
                    ...
                    using (DataTable DT = new DataTable())
                    {
                        ...
                        if (AgreementStatusID != 2)
                        {
                            ...
                            if (TempDT.Rows.Count > 0)
                            {
                                for (int i = 0; i < TempDT.Rows.Count; i++)
                                {
                                    DataRow[] Rows = DT.Select("FrontsOrdersID = " + TempDT.Rows[i]["FrontsOrdersID"]);
                                    if (Rows.Count() > 0)
                                    {
                    ...
                    using (DataTable DT = new DataTable())
                    {
                        ...
                        if (AgreementStatusID != 2)
                        {
                            ...
                            if (TempDT.Rows.Count > 0)
                            {
                                ...
                                for (int i = 0; i < TempDT.Rows.Count; i++)
                                {
                                    DataRow[] Rows = DT.Select("DecorOrderID = " + TempDT.Rows[i]["DecorOrderID"]);
                                    if (Rows.Count() > 0)
                                    {
                                        for (int j = 0; j < DT.Columns.Count; j++)
                                        {
        ...
        public void MoveOrdersTo1(int MegaOrderID)
        {
            DataTable TempDT = new DataTable();
            string SelectCommand = @"SELECT * FROM NewMegaOrders WHERE MegaOrderID = " + MegaOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
        ...
        private void CompareRows(DataTable table1, DataTable table2)
        {
        ...
        public bool HasOrders(int MegaOrderID, bool IsSample)
        {
        ...
        public bool HasOrders(int[] MainOrderIDs, bool IsSample)
        {
            bool hasFronts = false;
            bool hasDecor = false;
            ...
            for (int i = 0; i < MainOrdersIDs.Count(); i++)
            {
                ...
                if (FilterOrders(MainOrdersIDs[i]))
                {
                    ...
                    for (int c = 0; c < DecorCatalogOrder.DecorProductsCount; c++)
                    {
                        ...
                        //Cell1 = sheet1.CreateRow(RowIndex + 1).CreateCell(1DisplayIndex++3);
                        //Cell1.SetCellValue(string.Empty);
                        //Cell1.CellStyle = SimpleHeaderCS;
    ...
    public class SendEmail
    {
        ...
        public void Send(int ClientID, string OrderNumbers, bool Save, string FileName)
        {
            ...
            using (MailMessage message = new MailMessage(from, to))
            {
                ...
                message.Body = "Отчет сгенерирован автоматически системой Infinium. Не нужно отвечать на это письмо. По всем вопросам обращайтесь " +
                ...
                try
                {
                    client.Send(message);
                    Success = true;
                    ClientEvents.AddEvent(ClientID, "Письмо-сверка успешно отправлено", "MarketingNewOrders");
                }

                catch (ArgumentException ex)
                {
                    ClientEvents.AddEvent(ClientID, $"ArgumentException: {ex.Message}", "MarketingNewOrders");
                    MessageBox.Show("ArgumentException\r\n" + ex.Message);
                }
                catch (InvalidOperationException ex)
                {
                    ClientEvents.AddEvent(ClientID, $"InvalidOperationException: {ex.Message}", "MarketingNewOrders");
                    MessageBox.Show("InvalidOperationException\r\n" + ex.Message);
                }
                catch (SmtpException ex)
                {
                    ClientEvents.AddEvent(ClientID, $"SmtpException: {ex.Message}", "MarketingNewOrders");
                    MessageBox.Show("SmtpException\r\n" + ex.Message);
                }
                catch (Exception ex)
                {
                    ClientEvents.AddEvent(ClientID, $"Exception: {ex.Message}", "MarketingNewOrders");
    ...
    public class BatchExcelReport
    {
        ...
        private int GetFCount(DataTable DT, bool Curved)
        {
            ...
            foreach (DataRow Row in DT.Rows)
            {
                if (Curved)
                {
                    if (Row["Square"] == DBNull.Value || Convert.ToDecimal(Row["Square"]) == 0)
    ...
    public class CopyMarketingOrders
    {
        private readonly DataTable ClientsDT = null;
        private readonly DataTable OrdersDT = null;
        private readonly DataTable toFrontsOrdersDT = null;
        private readonly DataTable toDecorOrdersDT = null;
        private readonly DataTable toMainOrdersDT = null;
        private readonly DataTable toMegaOrdersDT = null;
        private readonly DataTable fromFrontsOrdersDT = null;
        private readonly DataTable fromDecorOrdersDT = null;
        private readonly DataTable fromMainOrdersDT = null;
        private readonly DataTable fromMegaOrdersDT = null;

        public CopyMarketingOrders()
        {
            toFrontsOrdersDT = new DataTable();
            toDecorOrdersDT = new DataTable();
            toMainOrdersDT = new DataTable();
            toMegaOrdersDT = new DataTable();
            fromFrontsOrdersDT = new DataTable();
            fromDecorOrdersDT = new DataTable();
            fromMainOrdersDT = new DataTable();
            fromMegaOrdersDT = new DataTable();

            ClientsDT = new DataTable();
            using (SqlDataAdapter DA = new SqlDataAdapter("SELECT * FROM Clients WHERE Enabled = 1 ORDER BY ClientName",
                ConnectionStrings.MarketingReferenceConnectionString))
            {
                DA.Fill(ClientsDT);
            }
            ...
            using (SqlDataAdapter DA = new SqlDataAdapter("SELECT TOP 0 MegaOrderID, OrderNumber FROM NewMegaOrders ORDER BY OrderNumber",
                ConnectionStrings.MarketingOrdersConnectionString))
            {
                DA.Fill(OrdersDT);
            }
        ...
        public BindingSource ClientsBS
        {
            get
            {
                ...
                bs.DataSource = ClientsDT;
                return bs;
            }
        }

        public BindingSource OrdersBS
        {
            get
            {
                BindingSource bs = new BindingSource();
                bs.DataSource = OrdersDT;
                return bs;
            }
        }

        public bool GetOrdersDT(int ClientID)
        {
            OrdersDT.Clear();
            string SelectCommand = @"SELECT MegaOrderID, OrderNumber FROM NewMegaOrders WHERE ClientID=" + ClientID + " ORDER BY OrderNumber";
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                DA.Fill(OrdersDT);
            }
            return OrdersDT.Rows.Count > 0;
        }

        public void CopyMegaOrderToNew(int toClientID, int fromMegaOrderID)
        {
            GetFromMegaOrdersDT(fromMegaOrderID);
            if (fromMegaOrdersDT.Rows.Count > 0)
            {
                int MegaOrderID = CopyMegaOrder(toClientID);
                if (MegaOrderID != -1)
                {
                    GetFromMainOrders1DT(fromMegaOrderID);
                    foreach (DataRow zRow in fromMainOrdersDT.Rows)
                    {
                        int fromMainOrderID = Convert.ToInt32(zRow["MainOrderID"]);
                        int toMainOrderID = CreateMainOrder(fromMainOrderID, MegaOrderID, zRow);
                        if (toMainOrderID != -1)
                        {
                            GetFromFrontOrdersDT(fromMainOrderID);
                            if (fromFrontsOrdersDT.Rows.Count > 0)
                                CreateFrontsOrders(toMainOrderID);
                            GetFromDecorOrdersDT(fromMainOrderID);
                            if (fromDecorOrdersDT.Rows.Count > 0)
                                CreateDecorOrders(toMainOrderID);
                        }
                    }
                }
            }
        ...
        public void CopyMainOrderToNew(int toClientID, int fromMainOrderID)
        {
            int MegaOrderID = CreateMegaOrder(toClientID);
            if (MegaOrderID != -1)
            {
                GetFromMainOrders2DT(fromMainOrderID);
                int toMainOrderID = CreateMainOrders(MegaOrderID);
                if (toMainOrderID != -1)
                {
                    GetFromFrontOrdersDT(fromMainOrderID);
                    if (fromFrontsOrdersDT.Rows.Count > 0)
        ...
        public void CopyMainOrderToExist(int toClientID, int toMegaOrderID, int fromMainOrderID)
        {
            if (toMegaOrderID != -1)
            {
                GetFromMainOrders2DT(fromMainOrderID);
                int toMainOrderID = CreateMainOrders(toMegaOrderID);
                if (toMainOrderID != -1)
                {
                    GetFromFrontOrdersDT(fromMainOrderID);
                    if (fromFrontsOrdersDT.Rows.Count > 0)
        ...
        public void MoveMegaOrderToNew(int toClientID, int fromMegaOrderID)
        {
            string SelectCommand = @"SELECT * FROM NewMegaOrders WHERE MegaOrderID=" + fromMegaOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    using (DataTable DT = new DataTable())
                    {
                        ...
                        {
                            int OrderNumber = 1;
                            OrderNumber = GetNextOrderNumber(toClientID);
                            DT.Rows[0]["ClientID"] = toClientID;
                            DT.Rows[0]["OrderNumber"] = OrderNumber;
                            DA.Update(DT);
                        }
                    }
                }
        ...
        public void MoveMainOrderToNew(int toClientID, int fromMainOrderID)
        {
            int MegaOrderID = CreateMegaOrder(toClientID);

            string SelectCommand = @"SELECT * FROM NewMainOrders WHERE MainOrderID=" + fromMainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
        ...
        public void MoveMainOrderToExist(int toClientID, int toMegaOrderID, int fromMainOrderID)
        {
            string SelectCommand = @"SELECT * FROM NewMainOrders WHERE MainOrderID=" + fromMainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    using (DataTable DT = new DataTable())
                    {
                        if (DA.Fill(DT) > 0)
                        {
                            DT.Rows[0]["MegaOrderID"] = toMegaOrderID;
                            DA.Update(DT);
                        }
                    }
                }
        ...
        private void GetFromMegaOrdersDT(int MegaOrderID)
        {
            fromMegaOrdersDT.Clear();
            string SelectCommand = @"SELECT * FROM NewMegaOrders WHERE MegaOrderID=" + MegaOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                DA.Fill(fromMegaOrdersDT);
            }
        }

        private void GetFromMainOrders1DT(int MegaOrderID)
        {
            fromMainOrdersDT.Clear();
            string SelectCommand = @"SELECT * FROM NewMainOrders WHERE MegaOrderID=" + MegaOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                DA.Fill(fromMainOrdersDT);
            }
        }

        private void GetFromMainOrders2DT(int MainOrderID)
        {
            fromMainOrdersDT.Clear();
            string SelectCommand = @"SELECT * FROM NewMainOrders WHERE MainOrderID=" + MainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                DA.Fill(fromMainOrdersDT);
            }
        ...
        private void GetFromFrontOrdersDT(int MainOrderID)
        {
            fromFrontsOrdersDT.Clear();
            string SelectCommand = @"SELECT * FROM NewFrontsOrders WHERE MainOrderID=" + MainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                DA.Fill(fromFrontsOrdersDT);
            }
        }

        private void GetFromDecorOrdersDT(int MainOrderID)
        {
            fromDecorOrdersDT.Clear();
        ...
        private int CopyMegaOrder(int ClientID)
        {
            int MegaOrderID = -1;
            toMegaOrdersDT.Clear();
            string SelectCommand = @"SELECT TOP 1 * FROM NewMegaOrders WHERE ClientID=" + ClientID + " ORDER BY MegaOrderID DESC";
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    DA.Fill(toMegaOrdersDT);
                    for (int i = 0; i < fromMegaOrdersDT.Rows.Count; i++)
                    {
                        int OrderNumber = 1;
                        if (toMegaOrdersDT.Rows.Count > 0)
                            OrderNumber = GetNextOrderNumber(ClientID);
                        DataRow NewRow = toMegaOrdersDT.NewRow();
                        NewRow.ItemArray = fromMegaOrdersDT.Rows[i].ItemArray;
                        NewRow["OrderNumber"] = OrderNumber;
                        NewRow["ClientID"] = ClientID;
                        toMegaOrdersDT.Rows.Add(NewRow);
                    }
                    DA.Update(toMegaOrdersDT);
                    toMegaOrdersDT.Clear();
                    if (DA.Fill(toMegaOrdersDT) > 0)
                        MegaOrderID = Convert.ToInt32(toMegaOrdersDT.Rows[0]["MegaOrderID"]);
                }
            }
            return MegaOrderID;
        }

        private int CreateMegaOrder(int ClientID)
        {
            int MegaOrderID = -1;
            toMegaOrdersDT.Clear();
            string SelectCommand = @"SELECT TOP 1 * FROM NewMegaOrders WHERE ClientID=" + ClientID + " ORDER BY MegaOrderID DESC";
        ...
        private int CreateMainOrder(int fromMainOrderID, int MegaOrderID, DataRow row)
        {
            toMainOrdersDT.Clear();
            string SelectCommand = @"SELECT TOP 1 * FROM NewMainOrders WHERE MainOrderID=" + fromMainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    DA.Fill(toMainOrdersDT);
                    DataRow NewRow = toMainOrdersDT.NewRow();
                    NewRow.ItemArray = row.ItemArray;
                    NewRow["MegaOrderID"] = MegaOrderID;
                    toMainOrdersDT.Rows.Add(NewRow);
                    DA.Update(toMainOrdersDT);
                }
            }
            return GetLastMainOrderID();
        }

        private int GetLastMainOrderID()
        {
            using (SqlDataAdapter DA = new SqlDataAdapter("SELECT MainOrderID FROM NewMainOrders ORDER BY MainOrderID DESC",
                ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (DataTable DT = new DataTable())
                {
        ...
        private int CreateMainOrders(int MegaOrderID)
        {
            toMainOrdersDT.Clear();
            ...
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                ...
                {
                    DA.Fill(toMainOrdersDT);
                    for (int i = 0; i < fromMainOrdersDT.Rows.Count; i++)
                    {
                        DataRow NewRow = toMainOrdersDT.NewRow();
                        NewRow.ItemArray = fromMainOrdersDT.Rows[i].ItemArray;
                        NewRow["MegaOrderID"] = MegaOrderID;
                        toMainOrdersDT.Rows.Add(NewRow);
                    }
                    DA.Update(toMainOrdersDT);
        ...
        private void CreateFrontsOrders(int MainOrderID)
        {
            ...
            string SelectCommand = @"SELECT TOP 0 * FROM NewFrontsOrders WHERE MainOrderID=" + MainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    DA.Fill(toFrontsOrdersDT);
                    for (int i = 0; i < fromFrontsOrdersDT.Rows.Count; i++)
                    {
                        DataRow NewRow = toFrontsOrdersDT.NewRow();
                        NewRow.ItemArray = fromFrontsOrdersDT.Rows[i].ItemArray;
                        NewRow["MainOrderID"] = MainOrderID;
                        toFrontsOrdersDT.Rows.Add(NewRow);
                    }
                    DA.Update(toFrontsOrdersDT);
                }
            }
        }

        private void CreateDecorOrders(int MainOrderID)
        {
            toDecorOrdersDT.Clear();
            string SelectCommand = @"SELECT TOP 0 * FROM NewDecorOrders WHERE MainOrderID=" + MainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    DA.Fill(toDecorOrdersDT);
                    for (int i = 0; i < fromDecorOrdersDT.Rows.Count; i++)
                    {
                        DataRow NewRow = toDecorOrdersDT.NewRow();
                        NewRow.ItemArray = fromDecorOrdersDT.Rows[i].ItemArray;
                        NewRow["MainOrderID"] = MainOrderID;
                        toDecorOrdersDT.Rows.Add(NewRow);
                    }
                    DA.Update(toDecorOrdersDT);
                }
            }
        }
    }
    ...
    public class CreateOrdersFromExcel
    {
        ...
        private readonly DataTable toMainOrdersDT = null;
        private readonly DataTable toMegaOrdersDT = null;
        private readonly DataTable fromDecorConfigDT = null;
        private readonly DataTable fromFrontsConfigDT = null;

        private DataTable DecorFromExcelDT;
        private readonly DataTable FrontsFromExcelDT;
        private DataTable OrdersFromExcelDT;
        ...
        public CreateOrdersFromExcel()
        {
            ...
            toMainOrdersDT = new DataTable();
            toMegaOrdersDT = new DataTable();
            fromDecorConfigDT = new DataTable();
            fromFrontsConfigDT = new DataTable();
            OrdersFromExcelDT = new DataTable();

            using (SqlDataAdapter DA = new SqlDataAdapter("SELECT TOP 0 * FROM DecorConfig", ConnectionStrings.CatalogConnectionString))
            {
                DA.Fill(fromDecorConfigDT);
            }

            using (SqlDataAdapter DA = new SqlDataAdapter("SELECT TOP 0 * FROM FrontsConfig", ConnectionStrings.CatalogConnectionString))
            {
                DA.Fill(fromFrontsConfigDT);
            }
        ...
        public void GetDataFromExcel()
        {
            ...
            DecorFromExcelDT.Clear();
            OrdersFromExcelDT.Clear();
            ...
            try
            {
                foreach (string iterationRow in data)
                {
                    ...
                    DataRow newRow = FrontsFromExcelDT.NewRow();

                    for (int i = 0; i < rowData.Length; i++)
                    {
                        if (i >= FrontsFromExcelDT.Columns.Count) break;
                        if (rowData[i].Length > 0)
                            newRow[i] = rowData[i];
                    }
                    FrontsFromExcelDT.Rows.Add(newRow);
                }
            }
            catch (Exception)
            {
            ...
            {
                if (Convert.ToInt32(FrontsFromExcelDT.Rows[i]["Type"]) != 0)
                    FrontsFromExcelDT.Rows[i].Delete();
            }
            for (int i = DecorFromExcelDT.Rows.Count - 1; i >= 0; i--)
            {
                if (Convert.ToInt32(DecorFromExcelDT.Rows[i]["Type"]) != 1)
        ...
        private bool IsFrontsConfingAdded(int FrontConfigID)
        {
            return fromFrontsConfigDT.Select("FrontConfigID = " + FrontConfigID).Count() > 0;
        }

        private DataRow GetDecorConfigRow(int DecorConfigID)
        {
            return fromDecorConfigDT.Select("DecorConfigID = " + DecorConfigID)[0];
        }

        private DataRow GetFrontsConfigRow(int FrontConfigID)
        {
            return fromFrontsConfigDT.Select("FrontConfigID = " + FrontConfigID)[0];
        }
        ...
        private void AddDecorConfig(int DecorConfigID)
        {
            ...
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.CatalogConnectionString))
            {
                using (DataTable DT = new DataTable())
                {
                    ...
                    for (int i = 0; i < DT.Rows.Count; i++)
                    {
                        DataRow NewRow = fromDecorConfigDT.NewRow();
                        NewRow.ItemArray = DT.Rows[i].ItemArray;
                        fromDecorConfigDT.Rows.Add(NewRow);
                    }
                }
            }
        }
        ...
        private void AddFrontsConfig(int FrontConfigID)
        {
            ...
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.CatalogConnectionString))
            {
                using (DataTable DT = new DataTable())
                {
                    ...
                    for (int i = 0; i < DT.Rows.Count; i++)
                    {
                        DataRow NewRow = fromFrontsConfigDT.NewRow();
                        NewRow.ItemArray = DT.Rows[i].ItemArray;
                        fromFrontsConfigDT.Rows.Add(NewRow);
                    }
                }
            }
        }
        ...
        public void CreateOrders(int ClientID)
        {
            ...
            using (DataView DV = new DataView(OrdersFromExcelDT))
            {
                DT1 = DV.ToTable(true, new string[] { "MegaOrderID" });
            }

            for (int i = 0; i < DT1.Rows.Count; i++)
            {
                int MegaOrderID = Convert.ToInt32(DT1.Rows[i]["MegaOrderID"]);
                DateTime DocDateTime = Security.GetCurrentDate();
                ...
                for (int j = 0; j < DT2.Rows.Count; j++)
                {
                    ...
                    if (FrontsRows.Count() > 0)
                        CreateFrontsOrders(FrontsRows, DocDateTime);
                    DataRow[] DecorRows = DecorFromExcelDT.Select("MegaOrderID = " + MegaOrderID + " AND MainOrderID=" + MainOrderID);
                    if (DecorRows.Count() > 0)
                        CreateDecorOrders(DecorRows, DocDateTime);
                }
            }
        }
        ...
        private void CreateMegaOrder(int ClientID, DateTime DocDateTime)
        {
            CurrentMegaOrderID = -1;
            toMegaOrdersDT.Clear();
            string SelectCommand = @"SELECT TOP 1 * FROM NewMegaOrders WHERE ClientID=" + ClientID + " ORDER BY MegaOrderID DESC";
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    ...
                    NewRow["OrderNumber"] = OrderNumber;
                    NewRow["DelayOfPayment"] = 1;
                    NewRow["OrderDate"] = DocDateTime;
                    toMegaOrdersDT.Rows.Add(NewRow);

                    DA.Update(toMegaOrdersDT);
                    toMegaOrdersDT.Clear();
                    if (DA.Fill(toMegaOrdersDT) > 0)
                        CurrentMegaOrderID = Convert.ToInt32(toMegaOrdersDT.Rows[0]["MegaOrderID"]);
                }
            }
        }

        private void CreateNewMainOrder(DateTime DocDateTime, string Notes)
        {
            toMainOrdersDT.Clear();
            DataRow Row = toMainOrdersDT.NewRow();
                ...
                " WHERE MegaOrderID = " + CurrentMegaOrderID, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    DA.Update(toMainOrdersDT);
                    toMainOrdersDT.Clear();
                    if (DA.Fill(toMainOrdersDT) > 0)
                        CurrentMainOrderID = Convert.ToInt32(toMainOrdersDT.Rows[toMainOrdersDT.Rows.Count - 1]["MainOrderID"]);
                }
            }
        }
                ...
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    ...
                    for (int i = 0; i < rows.Count(); i++)
                    {
                        ...
                        if (!IsDecorConfingAdded(DecorConfigID))
                            AddDecorConfig(DecorConfigID);

                        DataRow DecorConfigRow = GetDecorConfigRow(DecorConfigID);

                        DataRow NewRow = toDecorOrdersDT.NewRow();

                        foreach (DataColumn c1 in toDecorOrdersDT.Columns)
                        {
                            foreach (DataColumn c2 in fromDecorConfigDT.Columns)
                            {
                                if (c1.ColumnName == c2.ColumnName)
                                    NewRow[c1.ColumnName] = DecorConfigRow[c2.ColumnName];
                            }
                        }
        ...
        private void CreateFrontsOrders(DataRow[] rows, DateTime DocDateTime)
        {
            toFrontsOrdersDT.Clear();
            string SelectCommand = @"SELECT TOP 0 * FROM NewFrontsOrders WHERE MainOrderID=" + CurrentMainOrderID;
            using (SqlDataAdapter DA = new SqlDataAdapter(SelectCommand, ConnectionStrings.MarketingOrdersConnectionString))
            {
                using (SqlCommandBuilder CB = new SqlCommandBuilder(DA))
                {
                    ...
                    for (int i = 0; i < rows.Count(); i++)
                    {
                        ...
                        if (!IsFrontsConfingAdded(FrontConfigID))
                            AddFrontsConfig(FrontConfigID);

                        DataRow FrontsConfigRow = GetFrontsConfigRow(FrontConfigID);

                        DataRow NewRow = toFrontsOrdersDT.NewRow();

                        foreach (DataColumn c1 in toFrontsOrdersDT.Columns)
                        {
                            foreach (DataColumn c2 in fromFrontsConfigDT.Columns)
                            {
                                if (c1.ColumnName == c2.ColumnName)
                                    NewRow[c1.ColumnName] = FrontsConfigRow[c2.ColumnName];
                            }
                        }
