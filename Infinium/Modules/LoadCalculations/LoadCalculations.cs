...
namespace Infinium.Modules.LoadCalculations
{
    ...
    public class LoadCalculations
    {
        ...
        public BindingSource ClientsNotConfirmedList { get; }

        public BindingSource ClientsForAgreedList { get; }

        public BindingSource ClientsAgreedList { get; }

        public BindingSource ClientsOnProductionList { get; }

        public BindingSource ClientsInProductionList { get; }

        public int RankCoef { get; set; } = 1;

        public decimal AgreedSumRank { get; private set; }

        public decimal NotConfirmedSumRank { get; private set; }

        public decimal ForAgreedSumRank { get; private set; }

        public decimal OnProductionSumRank { get; private set; }

        public decimal InProductionSumRank { get; private set; }

        public List<Sector> SectorsList { get; set; } = new();
        ...
        public void GroupBySectors(List<int> sectorsId)
        {
            ...
            foreach (var item in groupTotal)
            {
                ...
                if (sumRank != 0)
                    SectorsList[sectorIndex].Percent =
                        decimal.Round(sumTotal / sumRank * 100, 3, MidpointRounding.AwayFromZero);
            }
        }
        ...
        public void CalculateLoad(OrderStatus orderStatus)
        {
            ...
            for (var i = 0; i < _decorOrdersDt.Rows.Count; i++)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var index11 = decorName.IndexOf(" ", StringComparison.Ordinal);
...
and mainOrderId={mainOrderId}");
            ...
            decimal rate1, decimal rate2, decimal rate3, decimal rate4)
        {
            ...
            switch (measureId)
            {
                case 1:
                    if (height == -1)
                        measureCount = decimal.Round(length * width / 1000000, 3, MidpointRounding.AwayFromZero) *
                                       count;
                    if (length == -1)
                        measureCount = decimal.Round(height * width / 1000000, 3, MidpointRounding.AwayFromZero) *
                                       count;
                    break;
                case 2:
                    if (height != -1)
        ...
        {
            var filter =
                $"clientId={clientId} and orderNumber={orderNumber} and orderStatus={orderStatus} and sectorId={sectorId}";
...
inner join infiniu2_users.dbo.users as u on s.userid = u.userid";
        ...
        public class Machine
        {
            ...
            public BindingSource DataSourceNotConfirmed { get; set; }

            public BindingSource DataSourceForAgreed { get; set; }

            public BindingSource DataSourceAgreed { get; set; }

            public BindingSource DataSourceOnProduction { get; set; }

            public BindingSource DataSourceInProduction { get; set; }
        ...
        public class Sector
        {
            ...
            public decimal SumRank { get; set; }
