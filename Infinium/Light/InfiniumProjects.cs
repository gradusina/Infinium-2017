...
namespace Infinium
{
    public class InfiniumProjects
    {
        ...
        public byte[] GGGGetUserPhoto(int UserID)
        {
            ...
            if (FM1.FileExist(Configs.DocumentsPath + FileManager.GetPath("UsersPhoto") + "/" + Rows[0]["Photo"], Configs.FTPType))
            {
                try
                {
                        return FM1.ReadFile(Configs.DocumentsPath + FileManager.GetPath("UsersPhoto") + "/" + Rows[0]["Photo"],
                        Convert.ToInt64(Rows[0]["FileSize"]), Configs.FTPType);
                }
                catch (Exception ex)
                {
        ...
        public void UsersPhotodd()
        {
            using (var DA = new SqlDataAdapter("SELECT userId, Photo FROM users", ConnectionStrings.UsersConnectionString))
            {
                using (var CB = new SqlCommandBuilder(DA))
                {
                    using (var DT = new DataTable())
                    {
                        ...
                        {
                            if (Convert.ToInt32(DT.Rows[i]["userId"]) == -1)
                                continue;
                            DT.Rows[i]["Photo"] = GGGGetUserPhoto(Convert.ToInt32(DT.Rows[i]["userId"]));
                        }
                        DA.Update(DT);
                    }
                }
        ...
        public void FillProjects(int StateID, int GroupID, int UserID, int DepartmentID)
        {
            ...
            var firedUserExpr = " AuthorID IN (SELECT UserID FROM infiniu2_users.dbo.Users WHERE Fired<>1)";

            if (FillExpr.Length > 0)
                FillExpr += " AND " + firedUserExpr;
            else
                FillExpr = " WHERE " + firedUserExpr;
        ...
        public void FillProjects(string searchText)
        {
            ProjectsDataTable.Clear();
            
            string selectCommand = $"SELECT * FROM Projects";
            string fillExpr = "";
            string ArticleExpr = $" Article LIKE '%{searchText}%'";
            string ProjectNameExpr = $" ProjectName LIKE '%{searchText}%'";

            if (fillExpr.Length == 0)
                fillExpr += " WHERE " + ArticleExpr;
            else
                fillExpr += " OR " + ArticleExpr;

            if (fillExpr.Length == 0)
                fillExpr += " WHERE " + ProjectNameExpr;
            else
                fillExpr += " OR " + ProjectNameExpr;
            
            using (var DA = new SqlDataAdapter(selectCommand + fillExpr, ConnectionStrings.LightConnectionString))
            {
                DA.Fill(ProjectsDataTable);
            }
        }
        ...
        public void FillPropositions()
        {
            ...
            using (var DA = new SqlDataAdapter("SELECT * FROM Projects WHERE AuthorID IN (SELECT UserID FROM infiniu2_users.dbo.Users WHERE Fired<>1) and IsProposition = 1", ConnectionStrings.LightConnectionString))
            {
                DA.Fill(ProjectsDataTable);
            }
        }
        ...
        public int AddProject(TreeView TreeView, string ProjectName, string article, string ProjectDescription, int AuthorID, bool bProposition)
        {
            DateTime DateTime;
            ...
            using (var DA = new SqlDataAdapter("SELECT TOP 1 * FROM Projects ORDER BY Projectid DESC", ConnectionStrings.LightConnectionString))
            {
                using (var CB = new SqlCommandBuilder(DA))
                {
                    using (var DT = new DataTable())
                    {
                        if (DA.Fill(DT) > 0)
                        {
                            if (article.Length < 1)
                            {
                                DT.Rows[0]["Article"] = DT.Rows[0]["ProjectID"];
                                DA.Update(DT);
                            }
                        }
                    }
        ...
        public int EditProject(TreeView TreeView, bool bNewMembers, string ProjectName, string article, string ProjectDescription, int ProjectID, bool bProposition)
        {
                    ...
                    using (var DT = new DataTable())
                    {
                        ...
                        for (var i = 0; i < MembersTree.Nodes.Count; i++)
                        {
                            ...
                            for (var j = 0; j < MembersTree.Nodes[i].Nodes.Count; j++)
                            {
                                ...
                                if (MembersTree.Nodes[i].Nodes.Count == 1)
                                    bAll = false;
                            }

                            if (MembersTree.Nodes[i].Nodes.Count == 0)
                                bAll = false;
