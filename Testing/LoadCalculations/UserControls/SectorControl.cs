...
namespace Infinium.Catalog.UserControls
{
    public partial class SectorControl : UserControl
    {
        ...
        private readonly List<MachineControl> _machineControls;
        ...
        public SectorControl()
        {
            ...
            _machineControls = new List<MachineControl>();
        }
        ...
        {
            for (var i = 0; i < _machineControls.Count; i++)
            {
                _machineControls[i].Clear();
                _machineControls[i].Visible = false;
            }
        }
        ...
        public void AddMachines(List<LoadCalculations.Machine> machinesList)
        {
            ...
            {
                var machineId = machinesList[i].Id;
                if (_machineControls.FindIndex(m => m.MachineId == machineId) != -1)
                {
                    if (machinesList[i].Empty)
                    {
                        _machineControls[i].Visible = false;
                        continue;
                    }
                    _machineControls[i].Visible = true;

                    _machineControls[i].DataSource1 = machinesList[i].DataSource1;
                    _machineControls[i].DataSourceNotConfirmed = machinesList[i].DataSourceNotConfirmed;
                    _machineControls[i].DataSourceForAgreed = machinesList[i].DataSourceForAgreed;
                    _machineControls[i].DataSourceAgreed = machinesList[i].DataSourceAgreed;
                    _machineControls[i].DataSourceOnProduction = machinesList[i].DataSourceOnProduction;
                    _machineControls[i].DataSourceInProduction = machinesList[i].DataSourceInProduction;
                    _machineControls[i].SumTotal = machinesList[i].SumTotal.ToString();
                }
                else
                {
                    var machineControl = new MachineControl
                    {
                        ...
                        MachineId = machinesList[i].Id,
                        Name = "MachineControl" + machinesList[i].Id,
                        MachineName = machinesList[i].Name
                    };
                    machineControl.PForm = this;
                    machineControl.Dock = DockStyle.Left;
                    machineControl.Location = new Point(i * MachineControlWidth + MachineControlMargin, 5);
                    machineControl.Margin = new Padding(0, 0, 15, 0);

                    panel4.Controls.Add(machineControl);
                    _machineControls.Add(machineControl);
                }
            }
        }
