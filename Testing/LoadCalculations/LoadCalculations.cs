...
namespace Infinium.Modules.LoadCalculations
{
    ...
    public class LoadCalculations
    {
        ...
        private readonly DataTable _calculationsDt;
        private readonly DataTable _machinesPriorityDt;
        ...
        public LoadCalculations()
        {
            ...
            _machinesPriorityDt = new DataTable();
            ...
            _groupbyClientDt.Columns.Add("sumTotal", typeof(decimal));
            ...
            _calculationsDt = new DataTable();
            _calculationsDt.Columns.Add(new DataColumn("id", typeof(int)) { AutoIncrement = true });
            _calculationsDt.Columns.Add("userId", typeof(int));
            _calculationsDt.Columns.Add("machineId", typeof(int));
            _calculationsDt.Columns.Add("sectorId", typeof(int));
            _calculationsDt.Columns.Add("clientId", typeof(int));
            _calculationsDt.Columns.Add("orderNumber", typeof(int));
            _calculationsDt.Columns.Add("orderStatus", typeof(int));
            _calculationsDt.Columns.Add("orderDate", typeof(string));
            _calculationsDt.Columns.Add("count", typeof(int));
            _calculationsDt.Columns.Add("machineName", typeof(string));
            _calculationsDt.Columns.Add("decorName", typeof(string));
            _calculationsDt.Columns.Add("rate1", typeof(decimal));
            _calculationsDt.Columns.Add("rate2", typeof(decimal));
            _calculationsDt.Columns.Add("rate3", typeof(decimal));
            _calculationsDt.Columns.Add("rate4", typeof(decimal));
            _calculationsDt.Columns.Add("total1", typeof(decimal));
            _calculationsDt.Columns.Add("total2", typeof(decimal));
            _calculationsDt.Columns.Add("total3", typeof(decimal));
            _calculationsDt.Columns.Add("total4", typeof(decimal));
            _calculationsDt.Columns.Add("sumtotal", typeof(decimal));
            ...
            _clientsAgreedDt.Columns.Add("clientId", typeof(int));
        ...
        private int Priority(int sectorId, int machineId)
        {
            var priority = 1;

            var rows = _machinesPriorityDt.Select($"sectorId={sectorId} and machineId={machineId}");
            if (rows.Any())
        ...
        public List<Machine> GroupByMachines(int sectorId)
        {
            ...
            foreach (var machineId in distMachines)
            {
                ...
                var clients = _calculationsDt.AsEnumerable()
                    ...
                    .Select(s => new {
                        ...
                        s.Key.clientId,
                        s.Key.orderNumber,
                        s.Key.orderStatus)}).OrderBy(x => x.client.Name);
                ...
                {
                    var clientId = item1.client.ClientId;
                    var orderNumber = item1.client.OrderNumber;
                    var orderStatus = item1.client.OrderStatus;
                    var orderDate = item1.OrderDate;

                    var details = _calculationsDt.AsEnumerable()
                        .Where(row => row.Field<int>("sectorId") == sectorId
                                      ...
                                      && row.Field<string>("orderDate") == orderDate)
                        .Select(s => new
                        {
                    ...
                    foreach (var item2 in details)
                    {
                        ...
                        total3 += t3;
                        total4 += t4;
                        FillClientsDt(orderStatus, clientId, orderNumber, orderDate, t1 + t2 + t3 + t4);
                    }
                }
                ...
                machine.Empty = false;
                machine.Priority = Priority(sectorId, machineId);
            ...
            foreach (var item in distClients)
            {
                ...
                switch (item.client.OrderStatus)
                {
                    case (int)OrderStatus.NotConfirmed:
                        ...
                        newRow["sumTotal"] = OrderSumTotal(OrderStatus.NotConfirmed, item.client.ClientId, item.client.OrderNumber);
                    ...
                    case (int)OrderStatus.ForAgreed:
                        ...
                        newRow["orderNumber"] = item.client.OrderNumber;
                        newRow["OrderDate"] = item.OrderDate;
                        newRow["sumTotal"] = OrderSumTotal(OrderStatus.ForAgreed, item.client.ClientId, item.client.OrderNumber);
                    ...
                    case (int)OrderStatus.Agreed:
                        ...
                        newRow["orderNumber"] = item.client.OrderNumber;
                        newRow["OrderDate"] = item.OrderDate;
                        newRow["sumTotal"] = OrderSumTotal(OrderStatus.Agreed, item.client.ClientId, item.client.OrderNumber);
                    ...
                    case (int)OrderStatus.OnProduction:
                        ...
                        newRow["orderNumber"] = item.client.OrderNumber;
                        newRow["OrderDate"] = item.OrderDate;
                        newRow["sumTotal"] = OrderSumTotal(OrderStatus.OnProduction, item.client.ClientId, item.client.OrderNumber);
                    ...
                    case (int)OrderStatus.InProduction:
                        ...
                        newRow["orderNumber"] = item.client.OrderNumber;
                        newRow["OrderDate"] = item.OrderDate;
                        newRow["sumTotal"] = OrderSumTotal(OrderStatus.InProduction, item.client.ClientId, item.client.OrderNumber);
        ...
        private void FillClientsDt(int orderStatus, int clientId, int orderNumber, string orderDate, decimal sumTotal)
        {
            DataRow newRow;
            DataRow[] rows;
            switch (orderStatus)
            {
                case (int)OrderStatus.NotConfirmed:
                    rows = _clientsNotConfirmedDt.Select($@"clientId={clientId} and orderNumber={orderNumber}");
                    if (!rows.Any())
                    {
                        newRow = _clientsNotConfirmedDt.NewRow();
                        newRow["clientId"] = clientId;
                        newRow["clientName"] = ClientName(clientId);
                    ...
                    {
                        rows[0]["sumTotal"] = Convert.ToDecimal(rows[0]["sumTotal"]) + sumTotal;
                    }
                    break;

                case (int)OrderStatus.ForAgreed:
                    rows = _clientsForAgreedDt.Select($@"clientId={clientId} and orderNumber={orderNumber}");
                    if (!rows.Any())
                    {
                        newRow = _clientsForAgreedDt.NewRow();
                        newRow["clientId"] = clientId;
                ...
                case (int)OrderStatus.Agreed:
                    rows = _clientsAgreedDt.Select($@"clientId={clientId} and orderNumber={orderNumber}");
                    if (!rows.Any())
                    {
                        newRow = _clientsAgreedDt.NewRow();
                        newRow["clientId"] = clientId;
                ...
                case (int)OrderStatus.OnProduction:
                    rows = _clientsOnProductionDt.Select($@"clientId={clientId} and orderNumber={orderNumber}");
                    if (!rows.Any())
                    {
                        newRow = _clientsOnProductionDt.NewRow();
                        newRow["clientId"] = clientId;
                ...
                case (int)OrderStatus.InProduction:
                    rows = _clientsInProductionDt.Select($@"clientId={clientId} and orderNumber={orderNumber}");
                    if (!rows.Any())
                    {
                        newRow = _clientsInProductionDt.NewRow();
                        newRow["clientId"] = clientId;
        ...
        private decimal OrderSumTotal(OrderStatus orderStatus, int clientId, int orderNumber)
        {
            var sumTotal = _calculationsDt.AsEnumerable().
                Where(row => row.Field<int>("orderStatus") == (int)orderStatus
                             && row.Field<int>("clientId") == clientId
                             && row.Field<int>("OrderNumber") == orderNumber)
                .Select(x => x)
        ...
        public void CreateSectors(List<int> sectorsId)
        {
            ...
            {
                if (SectorsList.FindIndex(s => s.Id == sectorId) != -1)
                    continue;
        ...
        public void DeleteEmptySectors()
        {
            ...
            {
                var name = SectorsList[i].Name;
                SectorsList[i].Empty = !_calculationsDt.Select("sectorId=" + SectorsList[i].Id).Any();
            }
        }
        ...
        public void GroupBySectors(List<int> sectorsId)
        {
            ...
            var groupTotal = _calculationsDt.AsEnumerable()
...
and mainOrderId={mainOrderId}");
                    ...
                    AddCalculationDecorRow(userId, machineId, sectorId, clientId, orderNumber, orderDate, orderStatus,
...
and mainOrderId={mainOrderId}");
                    ...
                    AddCalculationFrontRow(userId, machineId, sectorId, clientId, orderNumber, orderDate, orderStatus,
        ...
        private void AddCalculationFrontRow(int userId, int machineId, int sectorId, int clientId, int orderNumber, string orderDate, OrderStatus orderStatus,
            ...
            decimal rate1, decimal rate2, decimal rate3, decimal rate4)
        {
            ...
            var newRow = _calculationsDt.NewRow();
            newRow["count"] = count;
            ...
            if (sumTotal != 0)
                newRow["sumTotal"] = sumTotal;
            _calculationsDt.Rows.Add(newRow);
        }

        private void AddCalculationDecorRow(int userId, int machineId, int sectorId, int clientId, int orderNumber, string orderDate, OrderStatus orderStatus,
            int decorId, int measureId, decimal length, decimal height, decimal width, decimal count, decimal countRate1,
            decimal rate1, decimal rate2, decimal rate3, decimal rate4)
        {
            decimal measureCount = 0;
            ...
            decimal total1, decimal total2, decimal total3, decimal total4)
        {
            ...
            rows[0]["sumTotal"] = Convert.ToDecimal(rows[0]["sumTotal"]) + total1 + total2 + total3 + total4;
        }
...
inner join sectors on subsectors.sectorid = sectors.sectorid";
        ...
        private void GetMachinePriority()
        {
            const string selectCommand =
                @"select * from MachinePriorityInHercules";

            _machinesPriorityDt.Clear();
            using (var da = new SqlDataAdapter(selectCommand, ConnectionStrings.CatalogConnectionString)) da.Fill(_machinesPriorityDt);
        }
...
inner join infiniu2_users.dbo.users as u on s.userid = u.userid";
        ...
        public class Machine : IComparable<Machine>
        {
            ...
            public int CompareTo(Machine compareMachine)
            {
                // A null value means that this object is greater.
                if (compareMachine == null)
                    return 1;

                else
                    return compareMachine.Priority.CompareTo(this.Priority);
            }
            public void Clear()
            {
                Empty = true;
                SumTotal = 0;
                DataSource1 = null;
                DataSourceNotConfirmed = null;
                DataSourceForAgreed = null;
                DataSourceAgreed = null;
                DataSourceOnProduction = null;
                DataSourceInProduction = null;
            }
            public bool Empty
            {
                get;
